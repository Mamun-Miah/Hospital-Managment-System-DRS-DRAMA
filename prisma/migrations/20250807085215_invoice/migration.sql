/*
  ...
  - The values [Partial,Full,Due] on the enum `Invoice_payment_method` will be removed. If these variants are still used in the database, this will fail.
  - The values [Bkash,Cash] on the enum `Invoice_payment_type` will be removed. If these variants are still used in the database, this will fail.
...
-- AlterTable
ALTER TABLE `invoice` MODIFY `payment_method` ENUM('Bkash', 'Cash') NULL,
    MODIFY `payment_type` ENUM('Partial', 'Full', 'Due') NULL;
